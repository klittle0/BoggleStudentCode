import java.util.ArrayList;
import java.util.Arrays;

public class Boggle {

    public static String[] findWords(char[][] board, String[] dictionary) {

        ArrayList<String> goodWords = new ArrayList<String>();

        // TODO: Complete the function findWords(). Add all words that are found both on the board
        //  and in the dictionary.

        TST possibleWords = new TST();
        // Convert the list into a sorted array of strings, then return the array.
        String[] sol = new String[goodWords.size()];
        goodWords.toArray(sol);
        Arrays.sort(sol);
        return sol;

        // Insert all real words into the TST
        for (String each : dictionary) {
            possibleWords.insert(each);
        }

        // Makes visited array of booleans that corresponds to the board
        boolean[][] visited = new boolean[board.length][board[0].length];

        ArrayList<String> sequences = new ArrayList<>();
        // Performs DFS, starting at every spot in the board
        String prefix = "";
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length; j++) {
                dfs(board, visited, i, j, prefix);
            }
        }
    }

    // Implements depth-first search
    // We are checking to see if words exist AS WE GO
    public static void dfs(char[][] board, boolean[][] visited, int i, int j, String prefix, TST possibleWords){
        // Base Case: if index is out of bounds
        if (i < 0 || i >= board.length || j < 0 || j >= board[0].length){
            return;
        }
        // If the prefix isn't in the dictionary
        if (!possibleWords.lookup(prefix)){
            return;
        }
        // 2nd Base case??
        // Mark current spot as visited & update prefix
        visited[i][j] = true;
        prefix += visited[i][j];

        // Recurse:
        dfs(board, visited, i - 1, j, prefix, possibleWords);
        dfs(board, visited, i + 1, j, prefix, possibleWords);
        dfs(board, visited, i, j - 1, prefix, possibleWords);
        dfs(board, visited, i, j + 1, prefix, possibleWords);

        // Unmark as visited
        visited[i][j] = false;

        // Need an array of "good words" that exist in dictionary. Need to be checking somewhere to see if the words are
        // in dictionary, and if yes, add them to good words array. Eventually, I will return good words array



    }
}

